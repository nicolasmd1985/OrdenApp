...
public class orders extends DBController {
    ...
    public ArrayList<HashMap<String, String>> get_orders(String user_id) {
        ...
        //crea lista
        wordList = new ArrayList<HashMap<String, String>>();
        String selectQuery = "SELECT id_order, customer_id, description, aux_order, address, city_id, install_time FROM orders where fk_user_id = "+user_id+" and finish = 0";
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                map.put("city_id", cursor.getString(5));
                map.put("install_time", cursor.getString(6));
                map.put("fk_user_id", user_id);
    ...
    public boolean insert_order(HashMap<String, String> queryValues, int manual) {
        ...
        values.put("phone_contact", queryValues.get("phone_contact"));
